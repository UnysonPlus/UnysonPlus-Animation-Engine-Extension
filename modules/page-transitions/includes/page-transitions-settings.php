...
add_filter( 'upw_anim_engine_module_tabs', function ( $tabs ) {
	...
	$pt_tile = function ( $file, $label ) use ( $pt_base ) {
		return array(
			'small' => array( 'src' => $pt_base . '/' . $file . '.svg', 'height' => 53 ),
	...
	$pt_transition = array(
		...
		'picker'       => array(
			'transition' => array(
				...
				'value'   => 'fade',
				'search'  => __( 'Search transitions…', 'fw' ),
				'layout'  => 'tabs',
				'choices' => upw_ae_group_tiles(
					...
					array(
						'grp_wipe' => array( 'label' => __( 'Wipe & Slide', 'fw' ), 'ids' => array( 'wipe', 'slide', 'diagonal', 'curtain', 'doors', 'split', 'bars', 'blinds', 'stripes' ) ),
						'grp_shape' => array( 'label' => __( 'Shape & Iris', 'fw' ), 'ids' => array( 'reveal', 'iris', 'shape', 'conic', 'morph' ) ),
						'grp_fade' => array( 'label' => __( 'Fade & Zoom', 'fw' ), 'ids' => array( 'fade', 'contentfade', 'zoom' ) ),
						'grp_fx' => array( 'label' => __( '3D & FX', 'fw' ), 'ids' => array( 'flip', 'rotate', 'glitch', 'pixels', 'checkerboard', 'ripple' ) ),
					)
				),
			),
		),
